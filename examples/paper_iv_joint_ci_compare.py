#!/usr/bin/env python3
"""Compare joint CI methods for the IV model calibrated to Figure 5(b) in g2.pdf."""
...
from examples.balke_pearl_closed_form_ci_compare import (
    LOWER_COEFFS,
    LOWER_CONSTS,
    UPPER_COEFFS,
    UPPER_CONSTS,
...
# X types: never(0,0), complier(0,1), defier(1,0), always(1,1)
X_TYPES = np.array([[0, 0], [0, 1], [1, 0], [1, 1]], dtype=int)
# Y types: never(0,0), positive(0,1), negative(1,0), always(1,1)
Y_TYPES = np.array([[0, 0], [0, 1], [1, 0], [1, 1]], dtype=int)

# Calibrated to match Figure 5(b) / Table 1 targets:
# true ATE=-0.25, true LATE=-0.36, BP bounds ~= [-0.5502, -0.1460]
...
def true_quantities() -> dict[str, float | np.ndarray]:
    ...
    y_eff = Y_TYPES[:, 1] - Y_TYPES[:, 0]
    ate = float(np.sum(JOINT_STRATA * y_eff[None, :]))
    comp_mass = float(JOINT_STRATA[1, :].sum())
    late = float(np.sum(JOINT_STRATA[1, :] * y_eff) / comp_mass)
    return {
        "lb": lb,
        "ub": ub,
        "ate": ate,
        ...
        "ub_candidates": ub_candidates,
    }


def run_one_rep(rep_id: int, n: int, b: int, alpha: float, gamma: float, seed: int) -> RepResult:
    ...
    oracle_lb_se = candidate_standard_error(data, coeffs=LOWER_COEFFS[oracle_lb_idx])
...
def save_plots(rows: list[dict[str, float]], out_dir: Path, stem: str) -> list[Path]:
    ...
    ax.set_title("Paper IV Model: Joint CI Coverage")
    ax.set_xlabel("n")
    ...
    for key, label, marker in methods:
        ...
        ax.plot(n_vals, vals, marker=marker, linewidth=2, label=label)
    ax.set_title("Paper IV Model: Joint CI Width")
    ax.set_xlabel("n")
...
def main() -> None:
    ...
    parser.add_argument("--r", type=int, default=200, help="Monte Carlo replications per n.")
    parser.add_argument("--b", type=int, default=500, help="Bootstrap/subsampling/posterior draws per replication.")
    parser.add_argument("--n-grid", type=str, default="1000,10000,100000", help="Comma-separated sample sizes.")
    ...
    for n_idx, n in enumerate(n_grid):
        ...
        if args.workers > 1:
            try:
                with ProcessPoolExecutor(max_workers=args.workers) as ex:
                    futs = [
                        ex.submit(run_one_rep, rep_id=i, n=n, b=args.b, alpha=args.alpha, gamma=args.subsample_gamma, seed=int(rep_seeds[i]))
                ...
                for i in range(args.r):
                    results.append(run_one_rep(rep_id=i, n=n, b=args.b, alpha=args.alpha, gamma=args.subsample_gamma, seed=int(rep_seeds[i])))
                    completed_jobs += 1
        else:
            for i in range(args.r):
                results.append(run_one_rep(rep_id=i, n=n, b=args.b, alpha=args.alpha, gamma=args.subsample_gamma, seed=int(rep_seeds[i])))
        ...
        print(f"elapsed_for_n={n}:{elapsed_n:.1f}s", flush=True)
