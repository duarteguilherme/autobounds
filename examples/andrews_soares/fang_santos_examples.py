"""Run Fang-Santos-style examples with configurable DGPs."""
...
from simulation import SimulationConfig, plot_results, simulate_config
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run Fang-Santos examples for min(mu) CIs."
    )
    parser.add_argument(
        "--examples",
        default="all",
    ...
    parser.add_argument(
        "--one-sided",
        action="store_true",
        help="Use one-sided lower intervals (default alpha 0.025 recommended).",
    )
...
def main():
    ...
    for example in examples:
        ...
        if args.one_sided:
            ...
            plot_results(df, output_dir, prefix=prefix)
